...
function M.get_dependency_path(dirname) return M.opt .. '/' .. dirname end

function M.get_dependency_meta_path(dirname) return M.meta .. '/' .. dirname end
...
function M.bashescape(str)
    ...
    if open then table.insert(ret, "'") end
...
function M.clone_repo(repo, target, recurse)
    ...
    if recurse then clonecmd = clonecmd .. "--recurse-submodules " end
...
function M.clone_repo_a(repo, target, recurse)
    print("cloning repository " .. repo .. " to " .. target)
    local clonecmd = {'git', 'clone'}
    if recurse then table.insert(clonecmd, "--recurse-submodules") end
...
local install_dependency_async = a.syncwrap(function(v)
    ...
    if not M.file_exists(metafilepath) then
        ...
        if not dependency_check then
            ...
            local recurse = false
            if v.recurse_submodules then recurse = true end
            ...
            if ret == 0 then
                if type(v.buildcmd) == "string" then
                    ...
                    ret = assert(a.spawn_lines_a({v.buildcmd, cwd = target},
                                                 print))
                    if ret == 0 then success = true end
            ...
            if success and v.launchscript ~= nil then
                ...
                if scriptname == nil then scriptname = v.dirname end
                if type(scriptname) ~= 'string' then
                    ...
                    elseif type(script) ~= 'string' then
                        ...
                        M.bashexec("echo " .. M.bashescape(script) .. " > " ..
                                       M.bashescape(scriptpath))
...
function M.exec_checker(tool)
    return function()
        if a.spawn_a {tool, "--version"} == nil then return tool end
    end
end
    ...
    return function(step)
        return a.run(function()
            ...
            local ret, err = a.spawn_lines_a({"nproc"}, function(line)
                if line then proc = tonumber(line) end
            end)
            if ret == 0 then
                if proc then
                    return step(proc)
