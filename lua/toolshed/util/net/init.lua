...
function M.http_async(url, opts)
    ...
    if opts.configpath ~= nil and type(opts.configpath) ~= 'string' then
        error('Invalid configpath type. Expected string but got ' .. type(opts.configpath))
    end
    ...
    if opts.configpath then
        table.insert(cmd, '-K')
        table.insert(cmd, opts.configpath)
    end
    for k, v in pairs(opts.headers) do
        table.insert(cmd, '-H')
    ...
    return function(callback)
        ...
        a.run(function()
            ...
            if err == 0 then
                ...
                local headers = {}
                for x in string.lines(rxerr) do
