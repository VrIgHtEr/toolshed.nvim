...
local function sink(q, index)
    ...
    while index <= max do
        ...
        if q[index] >= q[child] then return end
...
local function float(q, index)
    while index > 1 do
        ...
        if q[index] <= q[parent] then return end
...
function M.new(tbl)
    ...
    if tbl ~= nil then
        if type(tbl) ~= "table" then error "tbl must be a table" end
        for i, x in ipairs(tbl) do
            q[i] = x
            q.size = q.size + 1
        end
        for i = math.floor(q.size / 2), 1, -1 do sink(q, i) end
    end
    return q
end
