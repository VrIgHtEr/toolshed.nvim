...
a.sync = a.wrap(a.run)
function a.wait(defer)
    return coroutine.yield(defer)
end
function a.wait_all(defer)
    return coroutine.yield(a.join(defer))
end
function a.main_loop()
    return a.wait(function(step)
...
function a.spawn_lines_async(var, cb_stdout, cb_stderr)
    if cb_stdout then
        var.stdout = wrap_spawn_callback(cb_stdout)
    end
    if cb_stderr then
        var.stderr = wrap_spawn_callback(cb_stderr)
    end
...
function a.spawn_async(var)
    ...
    elseif type(var) == 'table' then
        ...
        cmd = tostring(var[1])
        for i = 2, len do
            args[i - 1] = tostring(var[i])
        end
        if var.cwd ~= nil then
            cwd = tostring(var.cwd)
        end
...
function a.getaddrinfo_async(host, service, hints)
    return function(step)
        local ret = vim.loop.getaddrinfo(host, service, hints, function(err, addresses)
            if err then
                step(nil, err)
            else
                step(addresses)
