...
local function create_entries(plugins)
    ...
    for k, v in pairs(plugins) do
        ...
        if v.def.plugin_type == nil then
            entry.plugin_type = constants.type.plugin
        elseif type(v.def.plugin_type) ~= 'number' then
            error('Invalid plugin_type type. Expected number but got ' .. type(v.def.plugin_type))
        else
            entry.plugin_type = v.def.plugin_type
        end
        ...
        for k, v in pairs(plugs) do
            if k ~= url then
                if dependencies[k] then
                    v.before[url] = true
                    plugs[url].after[k] = true
                else
                    v.after[url] = true
                    plugs[url].before[k] = true
                end
            end
        end
    end
    ...
    table.sort(bump_order, function(a, b)
        if a.plugin_type < b.plugin_type then
            return true
        elseif a.plugin_type > b.plugin_type then
            return false
        end
        return a.url:upper() < b.url:upper()
    end)
