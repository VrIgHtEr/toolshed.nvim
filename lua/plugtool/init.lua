...
local cache_plugin_name = "lewis6991/impatient.nvim"
...
local function discover(plugin, update)
	...
	if not plugdefs[url] then
		...
		if not config then
			...
			local reqpath = "plugtool.repository."
...
local function list_dirs(directory)
	local pfile = assert(
		io.popen(("find '%s' -mindepth 1 -maxdepth 1 -type d -printf '%%f\\0'"):format(directory), "r")
	)
